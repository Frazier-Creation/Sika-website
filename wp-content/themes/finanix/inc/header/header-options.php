<?php
    if ( class_exists( 'WooCommerce' ) && is_shop() || class_exists( 'WooCommerce' ) && is_product_tag()  || class_exists( 'WooCommerce' ) && is_product_category()  ) {
		...
		$header_width_meta = get_post_meta($finanix_shop_id, 'header_width_custom', true);
		$main_menu_hides   = !empty(get_post_meta($finanix_shop_id, 'menu-hides', true)) ? get_post_meta($finanix_shop_id, 'menu-hides', true) : '';
		$main_menu_type    = !empty(get_post_meta($finanix_shop_id, 'menu-type', true)) ? get_post_meta($finanix_shop_id, 'menu-type', true) : '';
		$rs_top_search     = get_post_meta($finanix_shop_id, 'select-search', true);
		$rs_show_cart      = get_post_meta($finanix_shop_id, 'show-cart-icon', true);
		$rs_offcanvas      = get_post_meta($finanix_shop_id, 'show-off-canvas', true);
		$rs_phoneicon      = get_post_meta($finanix_shop_id, 'show-off-phoneicon', true);
		$rs_show_quote     = get_post_meta($finanix_shop_id, 'show-quote', true); 
		$menu_bg           = get_post_meta($finanix_shop_id, 'menu-type-bg', true); 
		$menu_center_page  = get_post_meta($finanix_shop_id, 'center-menus', true);
		$quote_btn_texts   = get_post_meta($finanix_shop_id, 'quote_btn_text', true);
		
		$skew_styles       = get_post_meta($finanix_shop_id, 'show-skew', true);
		$skew_styles       = (!empty($skew_styles)) ? $skew_styles : '';
		
		$skew_style        = (!empty($finanix_option['skew_off'])) ? 'reactheme-skew-head' : '';
		//Topbar 
		$rs_top_bar        = get_post_meta($finanix_shop_id, 'select-top', true);
...
$main_menu_center = (!empty($finanix_option['main_menu_center'])) || ($menu_center_page == 'yes')  ? 'main-menu-center' : '';

$main_menu_icon = (!empty($finanix_option['main_menu_icon'])) ? 'main-menu-icon-hide' : '';

if ($header_width_meta != ''){
    $header_width = ( $header_width_meta == 'full' ) ? 'container-fluid': 'container';
}else{
    $header_width = !empty($finanix_option['header-grid']) ? $finanix_option['header-grid'] :'';

...
<body <?php body_class(); ?>>
	...
	?> 
    <?php if(!empty( $finanix_option['show-comingsoon']) && !is_user_logged_in()){
        get_template_part( 'coming-soon' );
        die();
    } ?>
    <?php 
    if(!empty($finanix_option['off_canvas'])){ ?>
    ...
    <?php        
        $extrapadding = !empty($finanix_option['show_call_btns']) ? '' : 'lesspadding';      
    ?>
